...
public class CharacterAction {
    ...
    public Player ChooseEnemy(JLabel label, JLabel label2, JLabel text, JLabel label3, int level) {
        if (currentEnemy > numberOfEnemyesInLocation) {
            startNewLocation(level);
        }
        if (currentEnemy == numberOfEnemyesInLocation) {
            return ChooseBoss(label, label2, text, label3, level);
        }
    ...
    public int[] EnemyBehavior(int k1, int k2, int k3, int k4, int k5, int k6, double i) {
        int arr[] = null;
        if (i < k1 * 0.01) {
            arr = kind_fight[0];
        }
    ...
    public int[] ChooseBehavior(Player enemy, CharacterAction action) {
        ...
        if (enemy instanceof Baraka) {
            arr = action.EnemyBehavior(15, 15, 60, 10, 0, 0, i);
        }
        if (enemy instanceof SubZero) {
            arr = action.EnemyBehavior(20, 20, 0, 50, 10, 0, i);
        }
        if (enemy instanceof LiuKang) {
            arr = action.EnemyBehavior(13, 13, 10, 64, 0, 0, i);
        }
        if (enemy instanceof SonyaBlade) {
            arr = action.EnemyBehavior(25, 25, 50, 0, 0, 0, i);
        }
        if (enemy instanceof ShaoKahn) {
            arr = action.EnemyBehavior(10, 40, 0, 40, 0, 10, i);
        }
        return arr;
    }
    ...
    public void AddPoints(Human human, Player[] enemyes, JDialog jDialog7, JDialog dialog) {
    ...
    public void AddPointsBoss(Human human, Player[] enemyes, JDialog jDialog7, JDialog dialog) {
        switch (human.getLevel()) {
            case 2:
                human.setExperience(30);
